    ...
    *filenames : variable length arguments of str
    ...
    def write(self, top=None, par=None, stream=None, **kwargs):
        ...
        if top is not None:
            ...
            f.write('*\n')
            self._write_top_to(f, True, False) # Don't write residue information (FL 11/05/21)
    ...
    def _write_str_to(self, f):
        ...
        self._write_top_to(f, True, True)
        f.write('\nread para card\n* Parameters generated by ParmEd\n*\n')
        self._write_par_to(f)

    def _write_top_to(self, f, write_version, write_resi):
        ...
        if write_resi:
            self._write_resi_to(f)

        if write_version:
            f.write('\nEND\n')

    def _write_resi_to(self, f):
        """
        Private method to write residue information to open file object
        :param f:
        :return:
        """
        for name in self.residues:
            res = self.residues[name]
            f.write(f'RESI {name} {res.net_charge:.3f}\n')
            for group in res.groups:
                f.write('GROUP\n')
                for atm in group:
                    f.write(f'ATOM  {atm.name:<5}  {atm.type:<7}  {atm.charge:7.3f}\n')
            f.write('\n')
            # Write Bonds
